...
| **Track B Phase 2 — pairing (firmware)** | ✅ hardware-verified | `CrossPointPairingActivity` (`src/activities/settings/`): one `POST <origin>/pair/start`, saves the minted `T` via `CrossPointContextStore`, renders QR (`QrUtils::drawQrCode`) of `verification_uri_complete` + nonce + the full URL (char-wrapped) + device label, exits — no poll; `silentRestart()` on exit. Menu item in `CrossPointContextSettingsActivity`. New `CrossPointContextClient::pairStart()` does the HTTPS POST + JSON parse. Paired + pushed end-to-end on device. Commit `d93d33f` |
| **Stage 4 — firmware cleanup** | 🟡 built, on-device test pending | Dropped `-DCROSSPOINT_DEFAULT_WRITE_TOKEN` + its `applyCompileTimeDefaults()` branch (no baked credential). Store now holds the **origin only**: `getNormalisedUrl()` strips any path, `CrossPointContextClient::postFile` appends `/ingest`, pairing appends `/pair/start`. Unified on `-DCROSSPOINT_DEFAULT_RELAY_URL` (now the MCP origin) and **retired the redundant `-DCROSSPOINT_DEFAULT_PAIRING_ORIGIN`** — pairing derives its origin from the store. Path-stripping means the existing paired device keeps working **without a re-pair**. **Compiles** (`pio run` SUCCESS, RAM 30.9%); **needs hardware**: one Send context round-trips on the deployed HTTPS path |
...
2. **Flash + confirm Stage 4 push.** Stage 4 (origin-only store, client appends `/ingest`,
   ...
   `pio run -t upload`, then reader → **Sync to CrossPoint Context** → expect "Context sent!"
