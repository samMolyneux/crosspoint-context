 ...
 * -DCROSSPOINT_DEFAULT_RELAY_URL default, or a self-hoster's override); pairing is only offered
 * when an origin is set. Manual URL + token entry remains the alternative provisioning path.
 */
class CrossPointPairingActivity final : public Activity {
 public:
  explicit CrossPointPairingActivity(GfxRenderer& renderer, MappedInputManager& mappedInput)
      : Activity("CrossPointPairing", renderer, mappedInput) {}
 ...
 private:
  ...
  CrossPointContextClient::PairResult pairResult;
